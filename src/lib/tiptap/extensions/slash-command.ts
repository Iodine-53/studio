...
import {
  Heading1, Heading2, Heading3, Pilcrow, Image, Table, List, ListOrdered, CheckSquare, CodeSquare, Minus, AlertTriangle, AreaChart, PenSquare, Rows, ListTodo, Film, SlidersHorizontal, Quote, FunctionSquare
} from "lucide-react";
...
const getCommandItems = (): CommandItem[] => [
  ...
  { title: "GeoGebra", icon: FunctionSquare, command: ({ editor, range }) => { editor.chain().focus().deleteRange(range).insertGeoGebra().run(); } },
];
